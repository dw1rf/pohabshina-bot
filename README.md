...
- **TTS**: озвучка сообщений пользователя в голосовом канале с очисткой слов-паразитов и соседних повторов.
...
## Фильтрация текста в TTS

- `/tts` включает озвучку сообщений одного пользователя, `/tts_say` озвучивает разовую фразу.
- Перед озвучкой бот удаляет `ну`, `короче`, `типа`, `как бы`, звуки `э/эм`, распространённые варианты написания и соседние повторы слов.
- В подписи к первому MP3 бот показывает фактически удалённые слова и их количество.
- Если после фильтрации текст стал пустым, бот озвучивает исходный очищенный текст без отчёта об удалениях.
...
- `BIRTHDAY_CHANNEL_ID` — канал для автоматических поздравлений с днем рождения (`0` = выключено).
- `TIMEZONE` — часовой пояс для проверки дней рождения, по умолчанию `Europe/Moscow`.
